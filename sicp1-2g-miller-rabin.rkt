...
;;    Rabin test.
;;
;; More precisely, we will show that the left-hand side is equivalent
;; to the right-hand side...
;;
;;              LHS                                          RHS
;;
;;  (A) Fermat-test value is NOT 1                  (C) Fermat-test value is NOT 1
;;  OR                                              OR
;;  (B) The first term of the Miller-Rabin   <==>   (D) There is a non-trivial square
;;  sequence is not 1 and none of the               root of 1 in the Miller-Rabin
;;  terms of the Miller-Rabin sequence              sequence.
;;  are -1 (mod n).
;;
;; To make this easier to talk about we introduced A, B, C, D:
;;
;;               LHS                                           RHS
;;    
;;             A or B                    <==>                 C or D
;;
;; If A is true then C is true and both LHS and RHS are true.
;; Likewise the other way around, when C is true A is true etc,
;; So we are really only interested in the cases where A and C
;; are both false.
;;
;; A and C are false means that the Fermat value
;; a^(n-1) = a^(k*2^e) = 1 mod n.
;;
;; (1) B is true.
;;
;;     Write the Miller-Rabin sequence like so:
;;
;;        (x * * * * * ... * y)
;;
;;     The Fermat value is 1 so some elements of
;;     the sequence are square roots of 1. B is true
;;     so no element of the sequence is -1.
;;     Therefore there is a square root that is
;;     either 1 or a non-trivial square root.
;;     But x is not 1 therefore there must be
;;     a non-trivial root somewhere in the sequence. 
;;     Therefore D is true.
;;
;; (2) D is true.
;;
;;     That means there is a non-trival root of 1
;;     somewhere in the Miller-Rabin sequence.
;;     Every element after this root must be 1.
;;     Every element before this root canno be
;;     -1 or 1, since this root is non-trivial.
;;     Therefore B is true.
;;
;; (3) B is false.
;;
;;     This means either x is 1 or there is
;;     a -1 somewhere in the sequence.
;;
;;        (x * * * * * ... *)
;;
;;     If x is 1 then there can be no non-trivial
;;     square roots of 1 anywhere in the sequence.
;;     If there is a -1 somewhere, then all terms
;;     before it cannot be roots, and all terms after
;;     it are 1, ie, trivial. Either way, there are
;;     no non-trivial roots and therefore D is false.
;;
;; (4) D is false.
;;
;;     There are no non-trivial roots in the Miller-Rabin
;;     sequence. But the Fermat value is 1, so either
;;     all terms are 1, or there is a term that
;;     is -1. Either way, B is false.
;;     
;; Therefore we see that whenever one side of LHS <==> RHS
;; is true, the other side is also true. And whenever one
;; side is false, the other side is false. Therefore LHS
;; and RHS are equivalent
;;
;; But a little bit earlier I said that the SICP exmpod-based
;; test is ALMOST equivalent to the textbook Miller-Rabin
;; test. There IS a difference. The SICP version examines
;; terms of the sequence for the computation of a^k, which
;; come before the Miller-Rabin sequence. So in principle
;; it may see a non-trivial root of 1 before it ever gets
;; to the Miller-Rabin part. Possibly this makes the SICP
;; formulation a little bit more powerful...

;; Exercise:
;;
;;    (* * * * * * * * * * * ... * * * * * * * * * * ... *)
;;     |<---- a^k sequence ----->| |<-- Miller-Rabin --->|
;;                                      sequence
;;
;; This is the sequence generated by examining the aguments
;; to the square(x) function in expmod. Is it possible for
;; a non-trivial square root of 1 to appear in the a^k
;; sequence, which does not also appear in the Miller-Rabin
;; sequence?
;;
;; Ok. That's enough analysis. This has been a tremendous
;; exercise in logical thinking. Finally we get to SICP 1.28!


;; Exercise 1.28 ========================================

;; No residue 1 <= a < n is congruent to 0 mod n, therefore
;; 0 can be used to unambiguously signal that a nontrivial
;; square root of 1 has been found.
;;
;; Since it can be proved (with a lot of fancy math) that
;; at least 3/4 of all 'a' are composite witnesses, the
;; chance that this test incorrectly determines n to be
;; prime after 10 trials is about 1 in 1 million.

(define (expmod-root-test base exp m)
  (define (square x)
    (let ((sq (modulo (* x x) m)))
      (if (and (not (= 1 x))
               (not (= (- m 1) x))
  ...
  (cond ((= exp 0) 1)
        ((even? exp)
         (square (expmod-root-test base (/ exp 2) m)))
        (else
         (modulo (* base (expmod-root-test base (- exp 1) m)) m))))

(define (miller-rabin-prime? n trials)
  (let loop ((tr trials))
    (cond ((= 0 tr)
           true)
          ((= 1 (expmod-root-test (random-integer 2 (- n 1))
                                  (- n 1)
                                  n))
           (loop (- tr 1)))
          (else
